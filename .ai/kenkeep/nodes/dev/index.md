...
- Open [**Hot-reload dev loop requires three concurrent processes**](dev/practice-hot-reload-dev-loop-requires-three-concurrent-processes.md) to learn about: Backend: ts-node via node --watch. Frontend: Vite at localhost:5173 with /api/* proxied to localhost:4317. No dist/ involvement. #dev #tooling #web #serve
...
- Open [**Start or reuse a dev:serve instance to visually debug the SPA**](dev/practice-start-or-reuse-dev-serve-instance-to-visually-debug-the-spa.md) to learn about: To see/screenshot the SPA, reuse or start dev:serve on :4317; rebuild dist-web for source changes, or use dev:web HMR at :5173. #web #serve #dev #debugging

## Components (what exists)
- Open [**npm run dev:serve — ts-node backend hot-reload script**](dev/map-npm-run-dev-serve-ts-node-backend-hot-reload-script.md) to learn about: Runs src/cli.ts serve via ts-node with node --watch; restarts on src/ changes; no dist/ involvement. #web #serve #scripts #dev
