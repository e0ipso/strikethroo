...
The build step (`scripts/build-skill-prompts.cjs`) compiles `src/skill-prompts/skills/<name>/SKILL.md.hbs` and writes assembled `SKILL.md` files to `templates/harness/skills/<name>/`, which is gitignored, untracked build output. Shared lookup material belongs in `src/skill-prompts/_references/*.md`; the assembler copies it unchanged into each skill's `references/` directory when the rendered prompt names it. Keep procedural rules in partials, where they remain inline in every consumer. The Git-tree channel serves the tracked root `skills/` release mirror instead, refreshed only by the release workflow's sync step.
