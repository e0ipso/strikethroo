...
---
The Archive UI control is a confirmation-gated button that appears on `done`-state plans across all three Plans views (List, Cards, Board). It uses the existing `Modal` primitive via an `ArchivePlanModal` component. On confirmation it calls `archivePlan()` in `src/web/data/api.ts`, and the SSE stream drives the automatic UI refresh after the server-side move completes.
