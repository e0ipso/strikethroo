...
`update` refreshes an initialized workspace via the same `init` conflict handling (no implicit `--force`), then runs `npx skills@1.5.24 update` for the seven workflow skills with `shell: false`, inherited input, and streamed output. The pinned installer checks project and global installations. Its zero-exit messages for missing installations, skipped skills, cancellations, and check failures count as incomplete updates. Exit 0 requires both workspace refresh and skills update to succeed; a workspace failure skips the installer, and an installer failure after a successful refresh keeps the refreshed workspace and exits 1 with recovery guidance. Users copy `npx strikethroo@latest update`.
