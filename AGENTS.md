...
During task generation (`st-generate-tasks` and the task-generation phase of `st-full-workflow`), each task is classified in-context into a named **execution profile** from the `execution_routing` section of `config/config.yaml` (guided by the `TASK_EXECUTION_ROUTING.md` hook), then the bundled `route-task-execution.cjs` helper deterministically writes the selected target as the task's exact `execution` frontmatter — the PR #53 contract (`model` required exact string, optional `harness`/`reasoning_effort`, no aliases/defaults/inheritance). `config.yaml` is the workspace's single generic structured configuration file: each feature claims one top-level section (never a dedicated per-feature YAML), and the routing loader ignores foreign sections. Profiles carry arbitrary user-defined names, mandatory LLM-facing descriptions, and non-empty target arrays; the default resolver filters each profile's targets to those whose harness CLI is installed (a target with no `harness` is native to the orchestrating harness and always available) and picks one uniformly at random, falling back to the first target when none are available — and one optional global resolver script (referenced from the section, stdin JSON → `{"selections": {"<taskId>": <candidateIndex>}}`) may instead pick a specific in-profile candidate by index. Profile names are never persisted; missing/duplicate/unknown assignments, malformed targets, and resolver failures abort routing atomically before blueprint generation (`POST_TASK_GENERATION_ALL` stays blueprint-only and runs after routing). The shipped section is `profiles: {}` (routing disabled — tasks get no `execution` metadata); the file is hash-tracked by `init` and editable through the Customize UI's Config tab (structured form; see Web SPA below). Source: `src/skill-scripts/shared/execution-routing.ts` + `src/skill-scripts/route-task-execution.ts`; prompt section: `src/skill-prompts/sections/task-execution-routing.md`.
...
│   │   ├── config.yaml            # Generic workspace configuration — one top-level section per
│   │   │                          #   feature (execution_routing ships disabled: profiles: {});
│   │   │                          #   editable via the Customize UI's Config tab
