...
Each `SKILL.md` is assembled at build time from Handlebars source templates in `src/skill-prompts/skills/<name>/SKILL.md.hbs`. Shared procedural blocks live in `src/skill-prompts/_partials/*.md.hbs`, referenced via `{{> name}}` and parameterized with call-site hash arguments (`{{> name heading="###"}}`); a genuine per-call-site behavioral divergence — never a value substitution — is expressed as a *slot*: a hash argument whose value is a block partial supplied at the call site with `{{#*inline "slot_name"}}...{{/inline}}`. Frontmatter carries exactly `name` + `description`, passed through verbatim; the source tree mirrors the output tree (`skills/<name>/SKILL.md.hbs` → `templates/harness/skills/<name>/SKILL.md`), so the output path is derived rather than declared — see Build Pipeline step 4 below for the render-in-place constraint this depends on. See `src/skill-prompts/README.md` for full assembly mechanics and `src/skill-prompts/AUTHORING.md` for the prompt-authoring house style (form-over-narrative, "no nuance clauses", the slot rule, anti-rationalization tables, Skill Discovery Optimization for descriptions, imperative phrasing) — read it before editing prompt content.
