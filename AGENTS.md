...
**The findings document has two delivery channels; the on-disk file wins**: the reviewer writes `review.xml` to the named path, and — only when it completed the review and the write itself failed — emits the same document between token-bearing delimiters on stdout. The stdout channel is read only when the canonical file is absent, and `runReviewRound` removes that one exact path before dispatch (never a glob, never `.self-review.yaml`, never a custom output name, and any prior `findings.json` is left in place), so file precedence always refers to the current invocation. A recovered document is written to the canonical path and enters the identical `validateAgainstSchema` call: the XSD stays the sole certification authority, there is no second parser, and nothing short-circuits into `parseReviewFindings`, whose linear tag scan is only safe behind validation. The delimiters carry a per-dispatch collision token and the extractor rejects a delimited region that does not begin with `<?xml` or `<review` — which is why the placeholder inside the dispatch prompt's own block is prose, so a reviewer echoing its instructions back cannot be read as a recovered document. The channel recovers a *completed* review whose delivery failed, and nothing more: a reviewer that could not inspect the repository remains uncertified, a failed recovery keeps the unchanged `findings-absent` outcome, and recovered XML that fails the schema stays schema-invalid. Neither degrades into a clean review, and `FindingsGateOutcome` and `RoundDecision` gain no members. Capture is requested by `dispatchReview` alone — the review path only — and it tees every chunk to this process's stderr so operator-visible progress survives; task dispatch output behavior is unchanged.
