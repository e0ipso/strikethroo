...
Immediately before delegation, `dispatch-task-execution.cjs` selects one target from the persisted profile. The `execution_routing.allow_external_harness_execution` boolean defaults to `false`, including when absent in existing configs. Before selection, only targets with no harness or the current harness are eligible; setting it to `true` includes external targets. When no target is eligible, dispatch reuses the native-default fallback. The code review gate is independent and still uses a second harness. The Config tab exposes the setting as **Allow external harness execution**. The built-in selector chooses the first eligible, non-avoided target in configuration order. An optional repository-relative selector receives one task's eligible candidates and accumulated avoid-set identifiers and returns one candidate identifier; override failures visibly fall back to current-harness defaults. Native/current-harness targets bypass configuration loading and readiness work. External readiness makes one request in a disposable Git workspace using the exact local `cli_args` and no model override. The harness must run a command that creates a nonce-bearing file, and Strikethroo verifies the contents before removing the workspace.
...
**Source**: `src/skill-scripts/code-review.ts` (entrypoint, the clean-skip branches, diff scoping, reviewer dispatch, `reviewedFieldsFor`, `_classify`), `src/skill-scripts/shared/review-findings.ts` (`xmllint` validation, the tag scanner, `countFindings`), `src/skill-scripts/shared/harness-discovery.ts` (configuration-aware ready harnesses, current one excluded), and `src/skill-scripts/capture-base-commit.ts` (records `<plan-dir>/review/base-commit.json` before phase execution). Reviewer dispatch reaches the harness through the model-optional path in `src/skill-scripts/shared/external-dispatch.ts`; `execution_routing` still requires an exact model on its own path.
...
`semantic-release` via `.github/workflows/release.yml`, triggered on push to `main`. The workflow runs `npm ci && npm run build && npx playwright install --with-deps chromium && npm test` (the browser install is needed because `npm test`'s e2e half runs against a real Chromium), then `npx semantic-release` with `HUSKY=0`. Inside semantic-release **prepare**, `@semantic-release/npm` bumps `package.json`, then `@semantic-release/exec`'s `prepareCmd` runs `npm run build:release-skills` so the root `skills/` tree is rebuilt from source with the new release version via esbuild's `SKILL_RELEASE_VERSION`, then `@semantic-release/git` commits `CHANGELOG.md`, `package.json`, `package-lock.json`, and `skills/**`. **Publish** follows: npm packs the rebuilt `dist/` and `dist-web/` and GitHub creates the release. `dist-test/` and `dist-web/` are deliberately excluded from the git commit — both are gitignored build output, and the SPA ships only through npm. Release commits are labeled `chore(release):` and carry `[skip ci]`.
