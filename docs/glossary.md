...
| **Execution profile** | A named, configuration-driven routing category in the `execution_routing` section of `config/config.yaml`. During task generation the LLM classifies each task into a profile by its description; a deterministic helper then writes the profile's selected target as the task's exact `execution` frontmatter. Profile names are transient and never persisted in tasks. |
