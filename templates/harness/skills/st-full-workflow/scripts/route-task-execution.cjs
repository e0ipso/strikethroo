...
// src/skill-scripts/shared/execution-policy.ts
var import_gray_matter = __toESM(require_gray_matter());
var fallback = (reason, detail) => ({ kind: "fallback", reason, detail });
var hasOwn = (value, key) => Object.prototype.hasOwnProperty.call(value, key);
var readTaskExecutionPolicy = (taskMarkdown, context) => {
  let data;
  try {
  ...
  const reasoningEffort = policy.reasoning_effort;
  if (!hasOwn(policy, "harness")) {
    return reasoningEffort === void 0 ? { kind: "native-override", model } : { kind: "native-override", model, reasoningEffort };
  }
  ...
  return reasoningEffort === void 0 ? { kind: "external-override", harness: policy.harness, model } : { kind: "external-override", harness: policy.harness, model, reasoningEffort };
};
...
var validateProfile = (name, raw, supportedHarnesses, errors) => {
  ...
  if ("resolver" in raw) {
    errors.push(
      `profile "${name}" configures a per-profile resolver; only the single top-level "resolver" is supported.`
    );
    return null;
  }
  for (const key of Object.keys(raw)) {
...
var validateAssignments = (rawMap, taskIds, profileNames) => {
  const errors = [];
  if (!isPlainObject2(rawMap)) {
    return {
      kind: "invalid",
...
var runCustomResolver = (scriptPath, input) => {
  try {
    const isNodeScript = /\.(cjs|mjs|js)$/.test(scriptPath);
    const stdout = isNodeScript ? (0, import_child_process.execFileSync)(process.execPath, [scriptPath], { input, encoding: "utf8" }) : (0, import_child_process.execFileSync)(scriptPath, { input, encoding: "utf8" });
    return { stdout };
  } catch (error) {
    return { error: error instanceof Error ? error.message : String(error) };
  }
};
var selectTargets = (config, assignments, options2) => {
  const profilesByName = new Map(config.profiles.map((p) => [p.name, p]));
  const tasks = [...assignments.entries()].sort(([a], [b]) => a - b).map(([id, profileName]) => {
  ...
  const request = JSON.stringify({ planId: options2.planId, tasks });
  ...
  if (!isPlainObject2(parsed) || !isPlainObject2(parsed.selections)) {
    return {
      kind: "resolver-failure",
  ...
  for (const task of tasks) {
    ...
    if (raw < 0 || !target) {
      return {
        kind: "resolver-failure",
        detail: `custom resolver selected candidate ${raw} for task ${task.id}, which is outside profile "${task.profile}"'s configured targets.`
      };
    }
...
var toExecutionMapping = (target, currentHarness) => {
  const execution = { model: target.model };
  if (target.harness !== void 0 && target.harness !== currentHarness) {
    execution.harness = target.harness;
  }
  if (target.reasoning_effort !== void 0) {
    execution.reasoning_effort = target.reasoning_effort;
  }
  return execution;
};
var writeExecutionFrontmatter = (taskMarkdown, execution) => {
  const match = taskMarkdown.match(/^(---\r?\n)([\s\S]*?)(\r?\n---)/);
  if (!match) throw new Error("missing frontmatter");
  const opening = match[1];
  const frontmatter = match[2];
  const closing = match[3];
  if (opening === void 0 || frontmatter === void 0 || closing === void 0) {
...
var usage = () => emit(
  {
    ...
    detail: "Usage: route-task-execution.cjs profiles <plan-id> | route-task-execution.cjs apply <plan-id> <assignment-json-file> <current-harness>"
  },
  2
);
...
var policyMatchesExecution = (policy, execution) => {
  if (execution.harness !== void 0) {
    return policy.kind === "external-override" && policy.harness === execution.harness && policy.model === execution.model && policy.reasoningEffort === execution.reasoning_effort;
  }
  return policy.kind === "native-override" && policy.model === execution.model && policy.reasoningEffort === execution.reasoning_effort;
};
var main = () => {
  const [mode, planIdArg, ...rest] = process.argv.slice(2);
  if (mode !== "profiles" && mode !== "apply" || !planIdArg) usage();
  const resolved = resolvePlan(planIdArg);
  if (!resolved) {
    ...
    emit(
      {
        ...
        customResolver: config.resolverScript !== void 0
      },
      0
    );
  }
  ...
  if (!SUPPORTED_HARNESSES.includes(currentHarness)) {
    emit(
      {
        kind: "infrastructure-failure",
  ...
  for (const task of documents.tasks) {
    ...
    const execution = toExecutionMapping(target, currentHarness);
    let next;
    try {
    ...
    if (!policyMatchesExecution(policy, execution)) {
      emit(
        {
          kind: "routing-failure",
          detail: `task ${task.id} would not verify after routing (${policy.kind}${"detail" in policy ? `: ${policy.detail}` : ""}).`
        },
        1
      );
      return;
  ...
  try {
    ...
    emit(
      {
        kind: "infrastructure-failure",
        detail: `routing aborted and rolled back: ${error instanceof Error ? error.message : String(error)}`
      },
      2
    );
    return;
